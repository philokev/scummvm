...
Bitmap16bppDecoder::Bitmap16bppDecoder(int width, int height, int bitsPerPixel) : Codec(),
		_width(width), _height(height), _bitsPerPixel(bitsPerPixel), _surface(nullptr) {
}

Bitmap16bppDecoder::~Bitmap16bppDecoder() {
	if (_surface.get()) {
		_surface->free();
	}
}

const Graphics::Surface *Bitmap16bppDecoder::decodeFrame(Common::SeekableReadStream &stream) {
	if (!_surface.get()) {
		_surface.reset(new Graphics::Surface());
	}
	...
	return _surface.release();
}

Graphics::PixelFormat Bitmap16bppDecoder::getPixelFormat() const {
